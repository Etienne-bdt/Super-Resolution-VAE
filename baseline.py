...
def evaluate_config(
    ...
    if lpips_fn is not None:
        ...
        plt.grid(True, alpha=0.3)
        plt.savefig(
            os.path.join(out_dir, "lpips_histogram.png"), dpi=300, bbox_inches="tight"
        )
        plt.close()
...
def evaluate_model_grid(
    ...
    base_out_dir: str,
    index_to_log: int = 980,
    bicubic_cache: np.ndarray | None = None,
) -> None:
    """
    ...
    for g1, rec, g2 in combos:
        cfg_dir = os.path.join(
            base_out_dir, f"g1-{int(g1)}_rec-{int(rec)}_g2-{int(g2)}"
        )
    ...
    try:
        ...
        print(
            f"Warning: failed to create per-model LPIPS barchart for {model_tag}: {e}"
        )
    ...
    try:
        _plot_per_model_barchart_mmse(
            summary_rows, base_out_dir, model_tag, metric="ssim"
        )
        _plot_per_model_barchart_mmse(
            summary_rows, base_out_dir, model_tag, metric="lpips"
        )
        _plot_per_model_barchart_mmse(
            summary_rows, base_out_dir, model_tag, metric="psnr"
        )
    except (ValueError, RuntimeError, OSError) as e:
        print(
            f"Warning: failed to create per-model MMSE barcharts for {model_tag}: {e}"
        )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate hyperparameter influence grid for Cond_VAE sampling"
    )
    parser.add_argument(
        "--laplacian_ckpt", type=str, help="Path to Laplacian model checkpoint"
    )
    parser.add_argument(
        "--gaussian_ckpt",
        type=str,
...
def main():
    ...
    # Data
    _, val_loader = init_dataloader(
        "s2v", batch_size=args.batch_size, patch_size=args.patch_size
    )
    ...
    if args.laplacian_ckpt is not None and os.path.exists(args.laplacian_ckpt):
        ...
        print(
            "No Laplacian checkpoint provided or path not found; skipping Laplacian model."
        )
    ...
    try:
        ...
        print(
            f"Saved global barchart to {os.path.join(results_root, 'global_barchart.png')}"
        )
    except (ValueError, RuntimeError, OSError) as e:
