...
def main():
    ...
    ckpt = torch.load("ckpt/3868451.pth", map_location="cpu")
    print(ckpt.keys())
    ...
    worst_rank = 0  # Change this to select different worst ranks
    best_rank = 0  # Change this to select different best ranks
    ...
    # Update filenames to include rank
    plt.imsave(
        f"results/{slurm_job_id}/worst_rank{worst_rank + 1}_x.png",
        worst_x[[2, 1, 0], :, :].clamp(0, 1).permute(1, 2, 0).cpu().numpy(),
    )
    plt.imsave(
        f"results/{slurm_job_id}/worst_rank{worst_rank + 1}_y.png",
        worst_y[[2, 1, 0], :, :].clamp(0, 1).permute(1, 2, 0).cpu().numpy(),
    )
    plt.imsave(
        f"results/{slurm_job_id}/worst_rank{worst_rank + 1}_model.png",
        model_worse[[2, 1, 0], :, :].clamp(0, 1).permute(1, 2, 0).cpu().numpy(),
    )
    plt.imsave(
        f"results/{slurm_job_id}/best_rank{best_rank + 1}_x.png",
        best_x[[2, 1, 0], :, :].clamp(0, 1).permute(1, 2, 0).cpu().numpy(),
    )
    plt.imsave(
        f"results/{slurm_job_id}/best_rank{best_rank + 1}_y.png",
        best_y[[2, 1, 0], :, :].clamp(0, 1).permute(1, 2, 0).cpu().numpy(),
    )
    plt.imsave(
        f"results/{slurm_job_id}/best_rank{best_rank + 1}_model.png",
        model_best[[2, 1, 0], :, :].clamp(0, 1).permute(1, 2, 0).cpu().numpy(),
    )
    save_img_histogram(
    ...
    print(
        f"Saved worst rank {worst_rank + 1} and best rank {best_rank + 1} images to results/{slurm_job_id}/"
    )
